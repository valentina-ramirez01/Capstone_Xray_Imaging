...
from PyQt6.QtCore import Qt, QTimer, QThread, pyqtSignal   # ⭐ added QThread + signal
...
class PiCamBackend:
    ...
    def ensure_running(self):
        if not self.ready or self.cam is None:
            log_event("Camera backend restarting (ensure_running)")
            self.start()
...
# ============================================================
# ⭐ XRAY THREAD WORKER — NEW (safest + minimal change)
# ============================================================
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        if sw1 == 0:
            ...
            log_event("Startup detected: TRAY OPEN")
            ...
            log_event("Startup detected: TRAY CLOSED")
            ...
            log_event("Startup detected: TRAY UNKNOWN")
    ...
    def banner(self, text, color=None):
        now = time.time()
        if now - self._last_banner_time < 0.05:
            return
    ...
    # ============================================================
    # ⭐ XRAY CAPTURE — THREADED (fixes blue banner)
    # ============================================================
    def on_xray(self):
        ...
        # ⭐ Launch XRAY in background thread
        import threading
        threading.Thread(target=self._do_xray, daemon=True).start()

    # ============================================================
    # XRAY WORKER THREAD — runs outside GUI
    # ============================================================
    def _do_xray(self):
        log_event("XRAY worker thread started")

        # Prevent other banners
        self.xraying = True

        # ----------------------------------------------------
        # ⭐ SHOW BLUE BANNER IMMEDIATELY
        # ----------------------------------------------------
        ...
        # ============================================================
        # UI UPDATE (must run in Qt thread)
        # ============================================================
        def finish_ui():
            self.all_leds_off()
            self.leds.write(self.leds.green, True)
