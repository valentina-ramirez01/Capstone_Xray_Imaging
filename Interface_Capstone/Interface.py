...
class MainWindow(QMainWindow):
    ...
    # ============================================================
    # ⭐ E-STOP: PRESS HANDLER — PATCHED (STRUCTURED, THREAD-SAFE)
    # ============================================================
    def handle_estop_fault(self):
        ...
        # Stop timers safely through GUI thread
        QTimer.singleShot(0, self.timer.stop)
        QTimer.singleShot(0, self.adc_timer.stop)
        QTimer.singleShot(0, self.align_timer.stop)

        # Stop HV and camera (OK to call from background thread)
        try: hv_off()
        except: pass
        ...
        # ⭐ GUI-thread block: LEDs, banner, button disabling
        def gui_updates():
            self.all_leds_off()
    ...
    # ============================================================
    # ⭐ E-STOP: RELEASE HANDLER — PATCHED (STRUCTURED, THREAD-SAFE)
    # ============================================================
    def handle_estop_release(self):

        log_event("E-STOP released — system re-enabled")

        # Restart camera backend (OK from background thread)
        try:
            log_event("PATCH A6 — Restarting camera backend after E-STOP")
            self.backend.start()
        except Exception as e:
            log_event(f"PATCH A6 ERROR — Camera failed to restart after E-STOP: {e}")

        # Restart timers through GUI thread
        QTimer.singleShot(0, self.adc_timer.start)
        QTimer.singleShot(0, self.align_timer.start)

        # ⭐ GUI-thread block for LEDs, banner, and re-enabling controls
        def gui_updates():
            ...
            # Restore preview if active before E-STOP
    ...
    # ============================================================
    # BANNER DISPLAY WITH RATE LIMITING
    # ============================================================
    ...
    def on_rotate45(self):
        ...
        if not self.has_closed_once or not self.armed:
            ...
            log_event("PATCH B4 — Rotation blocked (tray not closed)")
            return
    ...
    def on_show_last(self):
        ...
        log_event(f"PATCH B2 — Showing last X-Ray: {last_file}")
    ...
    def update_frame(self):
        ...
        try:
            ...
            log_event(f"PATCH A7 — grab_gray failed: {e}")
            return
    ...
    def closeEvent(self, event):
        ...
        try:
            with open("/tmp/xray_shutdown_flag", "w") as f:
                f.write("1")
            log_event("Shutdown: wrote /tmp/xray_shutdown_flag for daemon")
        except Exception as e:
            log_event(f"Shutdown: could not write shutdown flag: {e}")
        ...
        try:
            ...
            try:
                ...
                log_event(f"Shutdown: Motor1 close failed: {e}")
