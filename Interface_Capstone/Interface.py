...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.xraying         = False     # ⭐ Needed to protect XRAY banner

        self.preview_was_running_before_estop = False
        self._last_banner_time = 0  # Patch A4
        ...
        # UI Setup
        # --------------------------------------------------------
        self.alarm = QLabel("System Ready", alignment=Qt.AlignCenter)
        ...
        # --------------------------------------------------------
        # Detect tray position on startup
        # --------------------------------------------------------
        sw1 = GPIO.input(17)
        sw2 = GPIO.input(18)
    ...
    # ============================================================
    # E-STOP: PRESS HANDLER (banner-safe)
    # ============================================================
    ...
    # ============================================================
    # E-STOP: RELEASE HANDLER (banner-safe)
    # ============================================================
    def handle_estop_release(self):

        log_event("E-STOP released — re-enabling system")

        try:
            self.backend.start()
        except Exception as e:
            log_event(f"E-STOP CAMERA RESTART ERROR: {e}")
    ...
    def banner(self, text, color=None):
        ...
        self.alarm.repaint()
        QApplication.processEvents()   # ⭐ ensure it draws
    ...
    def check_adc_safety(self):
        ...
        log_event(f"ADC CHECK: HV={hv:.2f} V")

        if not ok:
            log_event(f"HV FAULT — {msg}")
    ...
    def on_rotate45(self):
        log_event("Rotate 45° clicked")

        if not self.has_closed_once or not self.armed:
            QMessageBox.warning(self, "Tray Not Closed",
                                "You must CLOSE the tray before rotating.")
            return

        if self.hv_fault_active:
            QMessageBox.warning(self, "HV Fault",
                                "Cannot rotate while HV fault is active.")
            return
    ...
    # ============================================================
    def on_home3(self):
        log_event("Home Rotation clicked")
    ...
    # ============================================================
    # ⭐ XRAY CAPTURE — FIXED BLUE BANNER + FIXED OVERWRITES
    # ============================================================
    def on_xray(self):
        log_event("XRAY capture initiated")
        ...
        # BLOCK alignment overwrites
        self.xraying = True

        # ⭐ Set LED + banner BEFORE blocking
        self.all_leds_off()
        self.leds.write(self.leds.blue, True)

        self.banner("HV On — Taking X-Ray Picture", color="blue")
        QApplication.processEvents()  # ⭐ MUST HAVE
        ...
        try:
            ...
            QMessageBox.critical(self, "Error",
                                 "Camera failure — HV OFF for safety.")
        ...
        log_event(f"Saved X-ray: {filename}")
    ...
    def on_show_last(self):
        ...
        files = sorted(
            glob.glob(base+"/*.jpg") + glob.glob(base+"/*.png"),
    ...
    def on_gallery(self):
        ...
        all_imgs = sorted(
            list(base_dir.glob("*.jpg")) + list(base_dir.glob("*.png"))
        )
    ...
    def on_editor(self):
        ...
        self.editor_window.setWindowFlag(Qt.Window, True)
    ...
    def closeEvent(self, event):
        ...
        try: motor1_forward_until_switch2()
        except: pass
